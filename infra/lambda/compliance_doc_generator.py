"""
AWS Lambda handler for NIST SP 800-88 Rev. 2 Certificate of Sanitization.

Receives enriched device data in the event payload, generates a PDF using
reportlab, uploads it to S3, and returns a pre-signed URL.
"""
import io
import json
import os
...
def lambda_handler(event, context):
    """
    Generate a NIST SP 800-88 compliance PDF and upload to S3.

    Event payload: enriched device dict (same shape as cert_item in devices.py).
    Returns: {"statusCode": 200, "body": {"pdf_url": "<pre-signed-url>"}}
    """
    try:
        device = event
        ...
        pdf_url = s3.generate_presigned_url(
            "get_object",
            Params={"Bucket": COMPLIANCE_BUCKET, "Key": s3_key},
            ExpiresIn=3600,
        )
