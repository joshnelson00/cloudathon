...
def seed_procedures():
    """Load procedures from JSON and insert into database."""
    ...
    print(f"\nSeeding {len(procedures)} procedures...")
...
def seed_devices():
    """Load devices from JSON and insert into database."""
    data_dir = Path(__file__).parent / "data"
    devices_file = data_dir / "cityserve-devices.json"

    if not devices_file.exists():
        print(f"Devices file not found: {devices_file} (skipping)")
        return True
    ...
    if not devices_data:
        print("No devices to seed\n")
        return True

    print(f"Seeding {len(devices_data)} devices...")
    ...
    for device_id, device in devices_data.items():
        try:
            ...
            print(f"  ERROR {device.get('chassis_serial', 'unknown')}: {e}")
            return False
...
def seed_users():
    """Load users from JSON and insert into database."""
    data_dir = Path(__file__).parent / "data"
    users_file = data_dir / "cityserve-users.json"

    if not users_file.exists():
        print(f"Users file not found: {users_file} (skipping)")
        return True
    ...
    for user in users:
        try:
            ...
            print(f"  ERROR {user.get('username', 'unknown')}: {e}")
            return False
